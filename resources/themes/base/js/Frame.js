...
function getStatus(){
		$.post('onlineuser.e',function(data){
			$("#footer").html( " 版权所有：中国电信 东方国信提供技术支持 , 在线用户数："+data)
		});
	}
function delCol(id){
...
function openMenu(menuId,menuName,url){
	$.ajax({  
	      type : "post",  
	      url : 'pages/frame/portal/menu/menuAction.jsp?eaction=checkResourceNameAndUrl',  
...
//	var topmenu = $("#navMenu").get(0);
//	var right=parseInt($("#globalMenu").width());
//	var left=parseInt($("#header-content h1").width());
//	
//	var menuWidth=$window.width()-right-left-70;
//	//alert("window:"+$window.width()+"left"+left+"  right"+right+"---menu"+menuWidth+"===="+topmenu.scrollWidth);
//	//alert(menuWidth);
//	$(topmenu).width(menuWidth);
//	if(topmenu.scrollWidth==$(topmenu).width()){
//		poor = 0;
//		//$(topmenu).width($(window).width());
//		$(".scrollerLeft,.scrollerRight").hide();
//	}else{
//		$(".scrollerLeft,.scrollerRight").show();
...
function hightNav(){
	...
	$("#navMenu a").click(function(){
		$(this).addClass("newHover").siblings("a").removeClass("newHover");
...
function openBaseState0(id,name,url,state,type){
	$.post('pages/frame/portal/menu/menuAction.jsp?eaction=LoadLeftTreeSub',{id:id},function(data){
...
function addTabs(id,tt,url){
	...
	if(ROPageState){
		...
		$tabDiv.tabs('add',{
			title:tt,
			content:'<iframe id="ContentIframe'+id+'" class="contentsIframe" src="'+url+'" style="position:relative"></iframe>',
			tools:[{
		...
		if(SysMenuType=='dropdown'){
			//alert($("#ContentIframe"+id).height());
			$("#ContentIframe"+id).css("height",($("#ContentIframe"+id).height())+"px");
		}
...
$(function(){
	...
	$("#navMenu a:first-child").addClass("newHover");
	$('#retrunR_btn').hide();
	...
	$(".favoritesWindowBtn02").click(function(){
		$("#favoritesWindow").fadeIn();
		$("#favoritesWindow ul li a").click(function(){
			$("#favoritesWindow").fadeOut();
		})
		...
		if(!CanLoginOutFlag){
	   	...
	   	if(confirm('您是否退出系统?')){
	   		if(IsPortal==''||IsPortal=='null'||IsPortal=='0'){
	   			window.location.href= appBase+'/logout.e';
				window.event.returnValue = false;    
	   		}else if(IsPortal=='1'){
	   			$.ajax({  
				      type : "post",  
				      url : appBase+'/logoutForReturn.e',  
				      data : {},  
				      async : false,  
				      success : function(data){  
				      	if(data=='1'){
	...
	$("#frame_ind").bind("click",function(){
		$("#indForm").remove();
		var formHtml = "<form id='indForm' method='post' action='"+appName+"/pages/frame/menuind/showInd.jsp' target='_blank'><input type='hidden' name='menuId' id='menuId' value='"+currentId+"'/></form>"
...
//		}
	...
	$("#open_collect").bind("click", function(e){
		$("#favoritesWindow").load("pages/frame/collect/CollectList.jsp",{},function(){
			$("#managerWindow").hide();
			var obj2 = $('.OpenFavoriteLink');
		    var X2 =obj2.offset().left;
		    //$("#favoritesWindow").css("left",X2-$("#favoritesWindow").width()/2+20);
		    $("#favoritesWindow").css("right","0");
	...
	$("#frame_manager").bind("click", function(e){
		var obj1 = $('.OpenUserLink');
    	var X1 =obj1.offset().left+8;
    	$("#managerWindow").css("left",X1).show();
	});
	
	
	//报表信息点击绑定
	$("#report_info").click(function(e){
		//alert(1);
		$("#reportInfoWindow").slideDown(100);
		$("#reportInfoWindow ul li a").click(function(){
			$("#reportInfoWindow").slideUp();
		});
		$("#reportInfoWindow").mouseleave(function(){
			$("#reportInfoWindow").slideUp();
		});
	});
	$("#report_info").bind("click", function(e){
		   $('#reportInfoWindow').css({top:(e.pageY+15)});
		   $('#reportInfoWindow').show();
	});
	
	
	//搜索栏绑定复选框
	$("#frame_serach").click(function(){
		$("#searchWindow").slideDown(100);
	...
	//
	$("#frame_query").bind("click", function(e){
	   $('#boncSearch').show();
	   $("#boncSearch").bind("mouseleave", function(e){
		   $(this).hide();
	   });
	});
});
 ...
 * @param type
 */
function openSubMenus(id,name,url,state,type){
	var $resLi = $("#resource_"+id);
	if(url!=""){
		openThisMenu(id,name,url,state,type);
		$resLi.addClass("selected").parent("li").siblings("li").find("a").removeClass("selected");
        $resLi.siblings("ul").slideToggle();
        $resLi.siblings("b").toggleClass("subscriptOver");
        $resLi.parent("li").siblings("li").find("ul").slideUp().siblings("b").removeClass("subscriptOver");
		return;
	}
	if($resLi.next("ul").html()==""){
		$.ajax({  
		      type : "post",  
		      url : 'pages/frame/portal/menu/menuAction.jsp?eaction=LoadLeftTree&pid='+id,  
		      data : {},  
		      async : false,  
		      success : function(data){ 
		    	  data = data.replace(/\r\n/g,'');
		          ...
		          $.each(menuData,function(index,menuItem){
		        	  if(menuItem.state=="leaf"){//无下级菜单
		        		  liHtml+="<li><a menuUrl='"+menuItem.attributes.url+"' id='resource_"+menuItem.id+"' href='javascript:void(0);' onClick=\"openThisMenu('"+menuItem.id+"','"+menuItem.text+"','"+menuItem.attributes.url+"','"+menuItem.attributes.menuState+"','"+menuItem.attributes.menuType+"')\">"+menuItem.text+"</a></li>";
		        	  }else{
		        		  liHtml+="<li><a menuUrl='"+menuItem.attributes.url+"' id='resource_"+menuItem.id+"' href='javascript:void(0);' onClick=\"openSubMenus('"+menuItem.id+"','"+menuItem.text+"','"+menuItem.attributes.url+"','"+menuItem.attributes.menuState+"','"+menuItem.attributes.menuType+"');\">"+menuItem.text+"</a><ul></ul></li>"
		        	  }
		          });
		          $resLi.next("ul").html(liHtml);
		          $resLi.next("ul").children("li").each(function(index,item){
...
function openThisMenu(id,name,url,menuState,menuType){
	if(url != null && url != '' && url != 'null'){
		if(menuType == '3'){
			if(url.indexOf("http://")==0){
				...
				window.open(ContextPathJs+'/'+url,"", 'width='+(window.screen.availWidth-10)+',height='+(window.screen.availHeight-30)+', top=0,left=0, toolbar=yes, menubar=yes, scrollbars=yes, resizable=yes,location=yes, status=yes');
			}
			return;
		}
		if(menuState=='1'){
		...
		$("#currentOpenMenu").text(name);
	}
}

function openSubMenuByShotcuts(id,name,url,state,type){
	$("#frameLayout").layout("expand","west");
	var $resLi = $("#resource_"+id);
	if(url!=""){
		openThisMenu(id,name,url,state,type);
	    $resLi.siblings("ul").slideDown();
	    $resLi.siblings("b").removeClass("subscriptOver");
	    $resLi.siblings("b").addClass("subscriptOver");
	    $resLi.parent("li").siblings("li").find("ul").slideUp().siblings("b").removeClass("subscriptOver");
		return;
	}
	if($resLi.next("ul").html()==""){
	...
	if(url==""||url==null||url==undefined){
		setTimeout(function(){openFirstUrlMenuId(id);},1000);
	}
}

function openFirstUrlMenuId(menuId){
	if($("#resource_"+menuId).parent("li").find(".subscript").size()>0){
		if($("#resource_"+menuId).next("ul").children("li").size()>0){
			menuId = $("#resource_"+menuId).next("ul").children("li").eq(0).find("a:eq(0)").attr("id");
		}
		menuId = menuId.substring(9,menuId.length);
		openFirstUrlMenuId(menuId);
	}else{
		var url = $("#resource_"+menuId).attr("menuUrl");
		if(url!="null"&&url!=null&url!=undefined&url!=""){
			$("#resource_"+menuId).click();
			$("#resource_"+menuId).addClass("selected").parent("li").siblings("li").find("a").removeClass("selected");
		}
	}
}